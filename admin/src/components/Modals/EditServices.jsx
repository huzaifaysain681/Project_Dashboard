...
const EditServiceModal = ({ service, ...props }) => {
  ...
  const addService = () => {
    ...
    setDisabled(true);
    postApi("services/detail", state).then((res) => {
      setDisabled(false);
      if (res?.success) {
        props.onHide();
      } else {
  ...
  const updateService = () => {
    if (!state.name && !state.type) return;
    setDisabled(true);
    putApi("services/detail", { _id: service?._id, ...state }).then((res) => {
      setDisabled(false);
      if (res?.success) {
        props.onHide();
      } else {
          ...
          <div style={styles.container}>
            ...
            <InputGroup className="m-1">
              ...
              >
                ...
                <option value={"pay-by-university"}>
                  {"University Sponsored"}
                </option>
                {/* <option value={"sponsor"}>{"Sponsored by someone"}</option>; */}
